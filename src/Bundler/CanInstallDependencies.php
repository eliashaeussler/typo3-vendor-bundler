...
trait CanInstallDependencies
{
    ...
    private function installVendorLibraries(bool $includeDevDependencies = false): Composer
    {
        $composer = $this->taskRunner->run(
            '🎶 Parsing composer.json of vendor libraries',
            fn () => $this->buildComposerInstance($this->librariesPath),
            Console\Output\OutputInterface::VERBOSITY_VERBOSE,
        );

        $this->taskRunner->run(
            '📦 Installing vendor libraries',
            function (TaskRunner\RunnerContext $context) use ($composer, $includeDevDependencies) {
